...
function renderInvites(_list){
    ...
    for(var i=0; _list[i]; i++){
        ...
        html += '<div class="message-content message-invite" data-id="'+_list[i].InviteMeshId+'">'+
            '        <img data-loading-url="'+_list[i].WorldImageUrl+'" class="message-image">'+
...
function AddInvite(_invite){
    ...
    var html = '<div class="message-content message-invite" data-id="'+_invite.InviteMeshId+'">'+
        '        <img data-loading-url="'+_invite.WorldImageUrl+'" class="message-image">'+
        '        <div class="message-text-wrapper">'+
...
'        <img data-loading-url="'+_friendrequest.UserImageUrl+'" class="message-image">'+
'        <div class="message-text-wrapper">'+
...
function generateInstanceHTML(_instance, _new){
    ...
    switch(_instance.InstanceRegion){
        case "0": region = "EU"; break;
        case "1": region = "US"; break;
        case "2": region = "AS"; break;
    }
    ...
    return '<div class="world-instance '+(_new === true?'new':'')+'" onclick="showInstanceDetails(\''+_instance.InstanceId+'\');">'+
                ...
                '    <div class="instanceRegion">'+region+'</div>'+
...
function LazyLoadImages() {
    ...
    var images = document.querySelectorAll('img[data-loading-url]');
    
    if (images.length > 0 && images[0].getAttribute('data-loading') != 'loading'){
        images[0].addEventListener('load', (event) => {
            event.target.removeAttribute('loading');
            event.target.removeAttribute('data-loading-url');
        });
        images[0].addEventListener('error', (event) => {
            event.target.removeAttribute('loading');
            event.target.removeAttribute('data-loading-url');
        });
        window.setTimeout(function(){
            images[0].removeAttribute('loading');
            images[0].removeAttribute('data-loading-url');
        }, 500);
        images[0].setAttribute('data-loading', 'loading');
        images[0].setAttribute('src', images[0].getAttribute('data-loading-url'));
    }
